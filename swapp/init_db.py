...
cursor=conn.cursor()
cursor.execute("CREATE TABLE IF NOT EXISTS planets (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(200) UNIQUE,gravity VARCHAR(200), climate VARCHAR(200), terrain VARCHAR(200),population BIGINT,url VARCHAR(200),date VARCHAR(40))")
cursor.execute("CREATE TABLE IF NOT EXISTS characters (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(200) UNIQUE,gender VARCHAR(20),homeworld VARCHAR(200),height SMALLINT,mass VARCHAR(20),date VARCHAR(40))")
